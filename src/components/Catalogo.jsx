        ...
        <div key={product.id} className='border-4 rounded-2xl w-4/12 flex justify-center flex-wrap font-bold bg-white border-pink-200 text-pink-200 mx-2 my-2 gap-5'>
          <div>
            ...
            <button
              ...
              className="flex flex-wrap my-4 px-6 py-2 bg-pink-200 text-white font-semibold rounded-2xl shadow-md hover:bg-pink-300 hover:scale-105 active:scale-95 transition-transform duration-200 ease-in-out"
              disabled={product.cantidad < 1}
            >
              {product.cantidad > 0 ? 'Agregar' : 'Agotado'}
            </button>
          </div>
          ...
          <img
            src={product.src}
            alt={product.nombre}
            className='w-40 h-40 flex justify-baseline object-cover rounded-xl'
          />
        </div>
      ))}
    </div>
